...
package com.weixin.sdk.oauth2.common.validators;
    ...
    public void validateClientAuthenticationCredentials(T request) throws OAuthProblemException {
        if (enforceClientAuthentication) {
            ...
            if (clientCreds == null || OAuthUtils.isEmpty(clientCreds[0]) || OAuthUtils.isEmpty(clientCreds[1])) {
                ...
                if (OAuthUtils.isEmpty(request.getParameter(OAuth.OAUTH_APPID))) {
                    missingParameters.add(OAuth.OAUTH_APPID);
                }
                if (OAuthUtils.isEmpty(request.getParameter(OAuth.OAUTH_APPSECRET))) {
                    missingParameters.add(OAuth.OAUTH_APPSECRET);
                }
            }
